...
/// Hardware interface for the BLE stack (nRF52840 implementation).
pub struct HwNRf52840 {}

impl HardwareInterface for HwNRf52840 {
...
const APP: () = {
    ...
    static mut BLE_LL: LinkLayer<HwNRf52840> = ();
    ...
    fn init() {
        ...
        let mut ll = LinkLayer::<HwNRf52840>::new(device_address, ble_timer);
        ...
        loop {
            if cfg!(feature = "log") {
                while let Ok(grant) = resources.LOG_SINK.read() {
                    for _ in grant.buf().chunks(255) {
                        //hstdout().unwrap().write_all(chunk);
